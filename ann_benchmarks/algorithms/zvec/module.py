...
Four adapters are provided, one per zvec search entry point, so the
...
* ``ZvecAnnBenchDocIds``  -> ``Collection.ann_bench_search_fast``
  (ann-benchmarks bypass: cached indexers, params set once like qsgngt).

All four share the same fit step: vectors are inserted in row order so the
...
``index`` method param). The Vamana adapter also exposes the native Flat
storage type independently from the graph quantizer: for example,
``quantize=uniform_uint4`` plus ``flat_data_type=uint8`` stores the original
SIFT byte values in the refine index; this is not UINT8 quantization.
"""
...
def _flatten_coupled_args(method_param: dict) -> dict:
    ...
    if not isinstance(coupled_args, dict):
        raise ValueError(
            "[zvec] coupled_args must resolve to one dictionary; configure it as a non-empty list of dictionaries"
        )
    conflicts = normalized.keys() & coupled_args.keys()
    if conflicts:
        raise ValueError(
            f"[zvec] duplicated method parameters between regular arguments and coupled_args: {sorted(conflicts)}"
        )
...
class ZvecBase(BaseANN):
    ...
    def __init__(self, metric: str, dim: int, method_param: dict):
        ...
        self._ef_construction = int(method_param.get("efConstruction", method_param.get("ef_construction", 500)))
        ...
        quantize = str(method_param.get("quantize", _DEFAULT_QUANTIZE[self._index_type])).lower()
        ...
        self._use_contiguous_memory = bool(method_param.get("use_contiguous_memory", True))
        ...
        if self._flat_data_type_name not in _FLAT_DATA_TYPE:
            raise ValueError(
                "[zvec] unsupported flat_data_type: "
                f"{self._flat_data_type_name}; expected one of "
                f"{sorted(_FLAT_DATA_TYPE)}"
            )
        self._flat_data_type = _FLAT_DATA_TYPE[self._flat_data_type_name]
        if self._flat_data_type_name == "uint8" and metric != "euclidean":
            raise ValueError("[zvec] raw uint8 Flat storage currently supports only L2")
        ...
        default_path = os.path.join(
            "zvec_indices",
            f"{self.interface}_{self._index_type}_{metric}_d{self._dim}_{self._build_tag()}_{quantize}",
        )
        explicit_path = method_param.get("index_path")
        self._path = os.path.abspath(os.path.expanduser(str(explicit_path))) if explicit_path else default_path
    ...
    def _build_tag(self) -> str:
        memory_tag = (
            ...
            f"_fdt{self._flat_data_type_name}"
        )
        if self._index_type == "vamana":
            ...
            return (
                ...
                f"_bulk{int(self._use_bulk_build)}"
    ...
    def _parse_query_spec(self, ef_or_spec, prefetch_offset=None, prefetch_lines=None) -> tuple[int, dict[str, int]]:
        if isinstance(ef_or_spec, dict):
            spec = ef_or_spec
            ef = int(spec["ef"])
            if "prefetch" in spec:
                if "prefetch_offset" in spec or "prefetch_lines" in spec:
                    raise ValueError("[zvec] prefetch cannot be combined with prefetch_offset/prefetch_lines")
                prefetch = spec["prefetch"]
                if not isinstance(prefetch, dict):
                    raise ValueError("[zvec] prefetch must be a mapping with offset and lines")
    ...
    def _open_readonly(self) -> None:
        self._collection = zvec_open(self._path, CollectionOption(read_only=True, enable_mmap=True))
    ...
    def load_index(self) -> bool:
        """Reuse a completed Collection without loading training vectors."""
    ...
    def set_query_arguments(self, ef_or_spec, prefetch_offset=None, prefetch_lines=None) -> None:
        self._ef, self._prefetch = self._parse_query_spec(ef_or_spec, prefetch_offset, prefetch_lines)
        self._query_param = self._make_query_param(self._ef)
        qtag = dict(self._prefetch) if self._prefetch else {}
        self.name = f"zvec-{self._label}({self._method_param}, ef={self._ef}" + (f", {qtag}" if qtag else "") + ")"
...
class ZvecFastQuery(ZvecBase):
    ...
    def _search(self, q: np.ndarray, n: int):
        ids, _scores = self._raw_obj.fast_query(VECTOR_FIELD, q, n, self._query_param)
...
class ZvecFastQueryDocIds(ZvecBase):
    ...
    ``fast_query_doc_ids`` and returns top-k ids after reranking with the
    configured native Flat storage type.
    ...
    def set_query_arguments(self, ef_or_spec, prefetch_offset=None, prefetch_lines=None) -> None:
        ...
        using_refiner = is_spec and (
            bool(ef_or_spec.get("refine", False)) or "candidate_topk" in ef_or_spec or "candidates" in ef_or_spec
        )
        ...
        candidate_values = {int(ef_or_spec[key]) for key in ("candidate_topk", "candidates") if key in ef_or_spec}
        if not candidate_values:
            raise ValueError("[zvec-refine] refine queries require candidate_topk or candidates")
        if len(candidate_values) != 1:
            raise ValueError("[zvec-refine] candidate_topk and candidates must be equal")
        ...
        if candidate_topk > ef:
            raise ValueError(f"[zvec-refine] candidate_topk ({candidate_topk}) cannot exceed ef ({ef})")
        ...
        self.name = (
            f"zvec-{self._label}({self._method_param}, ef={ef}, candidate_topk={candidate_topk}, prefetch={prefetch})"
        )
    ...
    def _search(self, q: np.ndarray, n: int):
        ...
        if self._with_scores:
            ids, _scores = self._raw_obj.fast_query_doc_ids(VECTOR_FIELD, q, n, self._query_param)
            return ids
    ...
    def _search_refine(self, q: np.ndarray, n: int):
        if self._candidate_topk is None:
            raise RuntimeError("[zvec-refine] set_query_arguments must be called before querying")
        if self._candidate_topk <= n:
            raise ValueError(
                f"[zvec-refine] candidate_topk ({self._candidate_topk}) must be greater than benchmark top-k ({n})"
            )
    ...
    def run_batch_query(self):
        if self._using_refiner:
            self._batch_res = [self._search(query, self._batch_n) for query in self._batch_X]
        else:
            self._batch_res = self._raw_obj.batch_fast_query_doc_ids_only(
                VECTOR_FIELD, self._batch_X, self._batch_n, self._query_param
            )
...
class ZvecAnnBenchDocIds(ZvecFastQueryDocIds):
    ...
    def set_query_arguments(self, ef_or_spec, prefetch_offset=None, prefetch_lines=None) -> None:
        ...
        self._label = "ann_bench_doc_ids_refine" if self._using_refiner else "ann_bench_doc_ids"
        self.name = (
    ...
    def _search_refine(self, q: np.ndarray, n: int):
        ...
        # Current wheels accept candidate_topk independently from the returned
        # top-k; retain compatibility with older staged wheels.
        if not hasattr(self, "_ann_bench_separate_refine_topk"):
            self._ann_bench_separate_refine_topk = True
        output_count = n if self._ann_bench_separate_refine_topk else self._candidate_topk
        if not hasattr(self, "_refine_out_buf") or len(self._refine_out_buf) != output_count:
            self._refine_out_buf = np.empty(output_count, dtype=np.int64)
        if self._ann_bench_separate_refine_topk:
            try:
                self._raw_obj.ann_bench_search_fast(q, self._refine_out_buf, self._candidate_topk)
            except TypeError:
                self._ann_bench_separate_refine_topk = False
